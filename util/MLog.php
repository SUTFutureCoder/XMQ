    ...
    private static function putLogIntoFile($strLogPath, $strLogType, $strLogMsg, $arrBackTrace = array()){
        ...
        if (empty($arrBackTrace)){
            ...
            $logBackTrace  = array(
                ...
                'args'     => isset($arrBackTrace[2]) ? json_encode($arrBackTrace[2]['args']) : json_encode($arrBackTrace[1]['args']),
            );
        } else {
            $logBackTrace  = array(
        ...
        //这里需要分为cli或apache/nginx模式
        $pathInfo = isset($_SERVER['PATH_INFO']) ? $_SERVER['PATH_INFO'] : CLIFILE;
        $userId   = isset(CoreConst::$userId)    ? CoreConst::$userId    : 0;

        $strLog   = sprintf("%s: %s [%s:%s] args[%s] logId[%s] uri[%s] Id[%s] %s" . PHP_EOL ,
            $strLogType,
            date('y-m-d H:i:s'),
            $logBackTrace['file'],
            $logBackTrace['line'],
